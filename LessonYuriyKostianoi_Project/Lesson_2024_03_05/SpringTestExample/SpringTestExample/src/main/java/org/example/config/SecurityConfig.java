...
import static org.example.entity.athorities.Authority.ROLE_READ;
import static org.example.entity.athorities.Authority.ROLE_WRITE;
        ...
        return http
                ...
                .authorizeHttpRequests(authorize ->
                        authorize.requestMatchers("/auth/login", "/auth/registration", "/error").permitAll() // какие url открыть
                                ...
                                .requestMatchers(HttpMethod.GET, "/users/**").hasAnyAuthority(ROLE_READ.name(), ROLE_WRITE.name()) // какие url открыть
                                .requestMatchers(HttpMethod.POST, "/users/**", "view/register").hasAuthority(ROLE_WRITE.name()) // какие url открыть
                                .requestMatchers(HttpMethod.PUT, "/users/**", "view/profile/edit").hasAuthority(ROLE_WRITE.name()) // какие url открыть
                                .requestMatchers(HttpMethod.DELETE, "/users/**").hasAuthority(ROLE_WRITE.name()) // какие url открыть
